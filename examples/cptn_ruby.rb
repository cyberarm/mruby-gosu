...
class Player
  ...
  def initialize(map, x, y)
    ...
    @standing, @walk1, @walk2, @jump = *Gosu::Image.load_tiles("#{ROOT_PATH}/media/cptn_ruby.png", 50, 50)
...
class Map
  ...
  def initialize(filename)
    ...
    @tileset = Gosu::Image.load_tiles("#{ROOT_PATH}/media/tileset.png", 60, 60, tileable: true)

    gem_img = Gosu::Image.new("#{ROOT_PATH}/media/gem.png")
    @gems = []

    lines = File.read(filename).each_line.map { |line| line.chomp }
    @height = lines.size
    @width = lines[0].size
...
class CptnRuby < Gosu::Window
  def initialize
    ...
    @sky = Gosu::Image.new("#{ROOT_PATH}/media/space.png", tileable: true)
    @map = Map.new("#{ROOT_PATH}/media/cptn_ruby_map.txt")
